...
impl<T, U, V> DocumentSigner<'_, '_, '_, T, U, V> {
    ...
    X: Serialize + SetSignature + TryMethod,
  {
    ...
    match method.key_type() {
      MethodType::Ed25519VerificationKey2018 => {
        // return a proper error when this crate gets its errors refactored
    ...
    S::Output: AsRef<[u8]>,
  {
    match self.merkle_key {
      Some((public, proof)) => {
        let proof: &Proof<D> = proof.downcast_ref().ok_or(Error::CoreError)?; // Change this when we get round to refactoring errors here. It used to be CoreError::InvalidKeyFormat
      ...
      None => Err(Error::CoreError), /* Change this when we get round to refactoring errors in this crate. Used to be
                                      * CoreError::InvalidKeyFormat */
    }
  }
}
