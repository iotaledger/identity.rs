...
async fn transfering_asset_works() -> anyhow::Result<()> {
  ...
  proposal.accept(TEST_GAS_BUDGET, &bob_client, &bob_signer).await?;
  ...
  proposal
    .conclude_or_cancel(TEST_GAS_BUDGET, &alice_client, &alice_signer)
    .await?;
...
async fn accepting_the_transfer_of_an_asset_requires_capability() -> anyhow::Result<()> {
  ...
  let (caty_client, caty_signer) = test_client.new_user_client().await?;
  ...
  let error = proposal
    .accept(TEST_GAS_BUDGET, &caty_client, &caty_signer)
    .await
    .unwrap_err();
...
async fn modifying_mutable_asset_works() -> anyhow::Result<()> {
  ...
  asset
    .set_content(420, TEST_GAS_BUDGET, &alice_client, &alice_signer)
    .await?;
...
async fn deleting_asset_works() -> anyhow::Result<()> {
  ...
  asset.delete(TEST_GAS_BUDGET, &alice_client, &alice_signer).await?;
