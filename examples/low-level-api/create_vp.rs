...
pub async fn create_vp() -> Result<Presentation> {
  ...
  doc_iss.sign_data(&mut credential, key_iss.private(), doc_iss.authentication().id())?;
  ...
  doc_sub.sign_data(&mut presentation, key_sub.private(), doc_sub.authentication().id())?;
