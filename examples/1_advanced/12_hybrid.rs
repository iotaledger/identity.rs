...
async fn create_did(client: &Client, secret_manager: &SecretManager, storage: &MemStorage, alg_id: CompositeAlgId) -> anyhow::Result<(Address, IotaDocument, String)> {
  ...
  let fragment = document.generate_method_hybrid(
    &storage, 
    alg_id, 
    None, 
...
async fn main() -> anyhow::Result<()> {
  ...
  let client: Client = Client::builder()
    ...
    create_did(&client, &mut secret_manager_issuer, &storage_issuer, CompositeAlgId::IdMldsa65Ed25519Sha512).await?;
    ...
    create_did(&client, &mut secret_manager_holder, &storage_holder, CompositeAlgId::IdMldsa65Ed25519Sha512).await?;
  ...
  // ===========================================================================
  println!("Sending presentation (as JWT) to the verifier: {}", presentation_jwt.as_str());
