...
pub enum Error {
  ...
  PresentationValidationError {
    error: identity_credential::validator::CompoundPresentationValidationError,
  },
  //TODO: IMPROVE ERROR
  #[error("{0}")]
  ...
  /// An attached handler attempted to resolve the DID, but the resolution did not succeed.
  #[error("attempted to resolve DID, but this action did not succeed")]
  HandlerError(Box<dyn std::error::Error + Send + Sync + 'static>),
}
