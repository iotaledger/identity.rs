  ...
  Tx: Transaction,
{
  ...
  async fn transaction_data<S>(&mut self, client: &impl CoreClient<S>) -> anyhow::Result<TransactionData> {
  ...
  async fn transaction_data_with_partial_gas<S>(
    &mut self,
    client: &impl CoreClient<S>,
  ...
  pub async fn with_sponsor<F, S>(mut self, client: &impl CoreClient<S>, sponsor_tx: F) -> Result<Self, Error>
  where
    S: Signer<IotaKeySignature> + OptionalSync,
  ...
  async fn get_or_init_programmable_tx<S>(
    &mut self,
    client: &impl CoreClient<S>,
  ) -> Result<&ProgrammableTransaction, Error> {
  ...
  pub async fn build<S>(mut self, client: &impl CoreClient<S>) -> Result<(TransactionData, Vec<Signature>, Tx), Error>
  where
    S: Signer<IotaKeySignature> + OptionalSync,
  {
  ...
  /// This method *DOES NOT* remove nor checks for invalid signatures.
  /// Transaction with invalid signatures will fail after attempting to execute them.
  pub async fn build_and_execute<S>(self, client: &impl CoreClient<S>) -> Result<TransactionOutput<Tx::Output>, Error>
  where
    S: Signer<IotaKeySignature> + OptionalSync,
  {
