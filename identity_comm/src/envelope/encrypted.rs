...
impl Envelope {
    ...
    pub fn anon_from_message<T>(message: &Message<T>, algorithm: Algorithm, recipients: &[PublicKey]) -> Result<Self>
    where
        T: Serialize,
    {
        Plaintext::from_message(message)
            .and_then(|plaintext| Self::anon_from_plaintext(&plaintext, algorithm, recipients))
    }
