...
impl CoreProfile {
  ...
  fn validate_aud<T>(&self, claims: &JwtClaims<T>) -> Result<()> {
    match (&self.rule_aud, claims.aud()) {
      ...
      (Validation::Required, _) => Err(ValidationError::MissingClaim("aud").into()),
      (Validation::Matching(_), _) => Err(ValidationError::InvalidAudience.into()),
    }
  }
  ...
  fn validate_iss<T>(&self, claims: &JwtClaims<T>) -> Result<()> {
    match (&self.rule_iss, claims.iss()) {
      ...
      (Validation::Required, _) => Err(ValidationError::MissingClaim("iss").into()),
      (Validation::Matching(_), _) => Err(ValidationError::InvalidIssuer.into()),
    }
  }
  ...
  fn validate_jti<T>(&self, claims: &JwtClaims<T>) -> Result<()> {
    match (&self.rule_jti, claims.jti()) {
      ...
      (Validation::Required, _) => Err(ValidationError::MissingClaim("jti").into()),
      (Validation::Matching(_), _) => Err(ValidationError::InvalidTokenId.into()),
    }
  }
  ...
  fn validate_sub<T>(&self, claims: &JwtClaims<T>) -> Result<()> {
    match (&self.rule_sub, claims.sub()) {
      ...
      (Validation::Required, _) => Err(ValidationError::MissingClaim("sub").into()),
      (Validation::Matching(_), _) => Err(ValidationError::InvalidSubject.into()),
    }
  }
  ...
  fn validate_timestamps<T>(&self, claims: &JwtClaims<T>) -> Result<()> {
    ...
    match (&self.rule_exp, claims.exp()) {
      ...
      (Validation::Required, None) => {
        return Err(ValidationError::MissingClaim("exp").into());
      }
    }
    ...
    match (&self.rule_nbf, claims.nbf()) {
      ...
      (Validation::Required, Some(_)) => {}
      (Validation::Required, None) => {
        return Err(ValidationError::MissingClaim("nbf").into());
      }
    }
    ...
    match (&self.rule_iat, claims.iat()) {
      ...
      (Validation::Required, Some(_)) => {}
      (Validation::Required, None) => {
        return Err(ValidationError::MissingClaim("iat").into());
      }
    }
