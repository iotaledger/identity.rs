...
impl JptPresentationValidatorUtils {
    ...
    pub fn extract_issuer_from_jpt<D>(presentation: &Jpt) -> std::result::Result<D, JwtValidationError>
    where
        D: DID,
        <D as FromStr>::Err: std::error::Error + Send + Sync + 'static,
    {

        let decoded = JwpPresentedDecoder::decode(presentation.as_str(), SerializationType::COMPACT).map_err(|err| JwtValidationError::JwpDecodingError(err))?;
        let claims = decoded.get_issuer_header().claims().ok_or("Claims not present").map_err(|err| JwtValidationError::CredentialStructure(crate::Error::JptClaimsSetDeserializationError(err.into())))?;
