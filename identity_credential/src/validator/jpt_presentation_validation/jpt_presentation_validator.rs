...
use identity_core::{convert::{ToJson, FromJson}, common::Url};
...
impl JptPresentationValidator {
        ...
        T: ToOwned<Owned = T> + serde::Serialize + serde::de::DeserializeOwned,
    {
        ...
        let aud: Option<Url> = decoded_jwp.get_presentation_protected_header().aud().map(|aud| {
            Url::from_str(aud).map_err(|_| JwtValidationError::JwsDecodingError(
                identity_verification::jose::error::Error::InvalidParam("invalid audience value"),
            )).ok()
        }).flatten();
