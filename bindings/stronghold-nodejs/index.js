...
switch (platform) {
  ...
  case 'win32':
    ...
    break
  case 'darwin':
    localFileExisted = existsSync(join(__dirname, 'identity-stronghold-nodejs.darwin-universal.node'))
    try {
      if (localFileExisted) {
        nativeBinding = require('./identity-stronghold-nodejs.darwin-universal.node')
      } else {
        nativeBinding = require('@iota/identity-stronghold-nodejs-darwin-universal')
      }
      break
    } catch {}
    switch (arch) {
