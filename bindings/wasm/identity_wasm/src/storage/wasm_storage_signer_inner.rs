...
/// Basically the same as the original storage signer with the exception, that this one here is owned.
/// Signer that offers signing capabilities for `Signer` trait from `secret_storage`.
/// `Storage` is used to sign.
pub struct StorageSignerOwned<K, I> {
  key_id: KeyId,
  public_key: Jwk,
  storage: Storage<K, I>,
}
...
impl<'a, K, I> StorageSignerOwned<K, I> {
...
#[async_trait(?Send)]
impl<K, I> Signer<IotaKeySignature> for StorageSignerOwned<K, I>
where
  K: JwkStorage + Sync,
  I: KeyIdStorage + Sync,
{
  ...
  async fn public_key(&self) -> core::result::Result<<IotaKeySignature as SignatureScheme>::PublicKey, SecretStorageError> {
  ...
  async fn sign(&self, data: &[u8]) -> core::result::Result<<IotaKeySignature as SignatureScheme>::Signature, SecretStorageError> {
    self
      .storage
      .key_storage()
