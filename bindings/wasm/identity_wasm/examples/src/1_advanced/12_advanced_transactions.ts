...
export async function advancedTransaction(): Promise<void> {
  ...
  const tx_response = await iotaClient.executeTransactionBlock({ transactionBlock: txDataBcs, signature: signatures, options: { showEffects: true }});
...
async function bobSponsorFn(tx_data: TransactionDataBuilder, client: IdentityClient): Promise<string> {
  const coin = await client.iotaClient().getCoins({ owner: client.senderAddress(), coinType: "0x2::iota::IOTA" }).then(res => res.data[0]);
