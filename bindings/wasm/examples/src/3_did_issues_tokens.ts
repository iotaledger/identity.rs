...
export async function didIssuesTokens() {
  ...
  // Publish the foundry.
  const [blockId, block] = await client.buildAndPostBlock(secretManager, { outputs: [authorityAliasOutput, carbonCreditsFoundry] });
  ...
  const aliasUnlockCondition: IImmutableAliasUnlockCondition = carbonCreditsFoundry.unlockConditions.find(unlockCondition => unlockCondition.type === IMMUTABLE_ALIAS_UNLOCK_CONDITION_TYPE)! as IImmutableAliasUnlockCondition;
