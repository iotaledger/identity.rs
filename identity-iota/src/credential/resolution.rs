...
pub(crate) async fn resolve_credential<T: Serialize, R: ?Sized + TangleResolve>(
  ...
  fail_fast: bool,
) -> std::result::Result<ResolvedCredential<T>, AccumulatedCredentialValidationError> {
  // simple converter
  let bool_to_result =
    |value: bool, error: StandaloneValidationError| -> Result<(), StandaloneValidationError> { value.then(|| ()).ok_or(error) };
  ...
  let initial_validator =
    |initial_event: InitialisationEvent<'_, T>, errors: &mut ValidationErrors| -> ControlFlow<()> {
      ...
      if let Err(error) = bool_to_result(
        ...
        StandaloneValidationError::IssuanceDate,
      ) {
        errors.push(error);
        if fail_fast {
          return ControlFlow::Break(());
      ...
      if let Err(error) = bool_to_result(
        ...
        StandaloneValidationError::ExpirationDate,
      ) {
        errors.push(error);
        if fail_fast {
          return ControlFlow::Break(());
  ...
  let issuer_event_validator =
    |resolved_issuer_event: ResolvedIssuerEvent<'_, T>, errors: &mut ValidationErrors| -> ControlFlow<()> {
      ...
      if let Err(verification_error) = resolved_issuer_doc
        ...
        .map_err(|error| StandaloneValidationError::IssuerProof {
    ...
    |resolved_subject_document_event: ResolvedSubjectEvent<'_>, errors: &mut ValidationErrors| -> ControlFlow<()> {
      if validation_options.allow_deactivated_subject_documents {
        ...
        if !subject_doc.document.active() {
          ...
          let err = StandaloneValidationError::DeactivatedSubjectDocument {
...
pub(crate) async fn resolve_presentation<T, U, R>(
  ...
  fail_fast: bool,
) -> std::result::Result<ResolvedPresentation<T, U>, AccumulatedPresentationValidationError>
where
  T: Serialize + Clone,
  U: Serialize + Clone,
  R: ?Sized + TangleResolve,
{
  let initial_validator = |initial_presentation_event: InitialPresentationEvent<'_, T, U>,
                           validation_errors: &mut Vec<StandaloneValidationError>|
   -> ControlFlow<()> {
    ...
    if let Err(error) = presentation.check_structure() {
      validation_errors.push(StandaloneValidationError::PresentationStructure(error));
      if fail_fast {
        return ControlFlow::Break(());
      }
    }
    if let Some((credential_position, _)) = presentation.non_transferable_violations().next() {
      validation_errors.push(StandaloneValidationError::NonTransferableViolation { credential_position });
      if fail_fast {
        return ControlFlow::Break(());
      }
    }
  ...
  let holder_validator = |resolved_holder_event: ResolvedHolderEvent<'_, T, U>,
   ...
   -> ControlFlow<()> {
    ...
    if let Err(error) = holder_doc.verify_data(
      ...
      validation_errors.push(StandaloneValidationError::HolderProof { source: error.into() });
      if fail_fast {
        return ControlFlow::Break(());
      }
    }
...
{
  let initial_validator =
    |_: InitialPresentationEvent<'_, T, U>, _: &mut Vec<StandaloneValidationError>| ControlFlow::Continue(());
  ...
  R: ?Sized + TangleResolve,
{
  ...
  let holder_validator =
    |_: ResolvedHolderEvent<'_, T, U>, _: &mut Vec<StandaloneValidationError>| -> ControlFlow<()> { ControlFlow::Continue(()) };
...
type ValidationErrors = OneOrMany<StandaloneValidationError>;
  ...
  T: Serialize,
{
  ...
  if is_break(initial_event_validator(
    ...
    return Err(AccumulatedCredentialValidationError { validation_errors });
  }
  ...
  let break_now = match resolve_document(resolver, issuer_url).await {
    ...
    Err(error) => {
      validation_errors.push(StandaloneValidationError::IssuerDocumentResolution { source: error.into() });
      fail_on_unresolved_issuer
    }
  };
  if break_now {
    ...
    .filter_map(|subject| subject.id.as_ref())
  {
    ...
    match resolved_subject_document {
      ...
      Err(error) => {
        validation_errors.push(StandaloneValidationError::SubjectDocumentResolution {
          source: error.into(),
          did_url: id.to_owned(),
        });
...
type CredentialResolutionErrors = BTreeMap<usize, AccumulatedCredentialValidationError>;
...
where
  ...
  U: Serialize + Clone,
  R: ?Sized + TangleResolve,
  I: Fn(InitialPresentationEvent<'_, T, U>, &mut Vec<StandaloneValidationError>) -> ControlFlow<()>,
  H: Fn(ResolvedHolderEvent<'_, T, U>, &mut Vec<StandaloneValidationError>) -> ControlFlow<()>,
  ...
  F: Future<Output = Result<ResolvedCredential<U>, AccumulatedCredentialValidationError>>,
{
  let mut presentation_resolution_error = AccumulatedPresentationValidationError {
    presentation_validation_errors: Vec::<StandaloneValidationError>::new(),
  ...
  match presentation
    ...
    .ok_or(StandaloneValidationError::MissingPresentationHolder)
  {
    Ok(holder_doc) => {
      ...
      match resolved_holder_doc {
        ...
        Err(error) => {
          presentation_validation_errors.push(StandaloneValidationError::HolderDocumentResolution { source: error.into() });
