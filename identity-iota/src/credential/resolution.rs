...
where
  ...
  G: Fn(Option<ResolvedIssuerEvent<'_, T>>, &mut ValidationErrors) -> ControlFlow<()>,
  H: Fn(Option<ResolvedSubjectEvent<'_>>, &mut ValidationErrors) -> ControlFlow<()>,
  T: Serialize,
{
  ...
  let break_now = match resolve_document(resolver, issuer_url).await {
    ...
    Err(error) => {
      ...
      is_break(issuer_event_validator(None, &mut validation_errors))
    }
  };
  if break_now {
    return Err(CredentialResolutionError { validation_errors });
    ...
    .filter_map(|subject| subject.id.as_ref())
  {
    ...
    match resolved_subject_document {
      ...
      Err(error) => {
        ...
        if is_break(subject_event_validator(None, &mut validation_errors)) {
          return Err(CredentialResolutionError { validation_errors });
        }
      }
    }
