...
pub(crate) fn txn_hash(txn: &BundledTransaction) -> Hash {
...
pub(crate) fn txn_hash_trytes(txn: &BundledTransaction) -> String {
  encode_trits(txn_hash(txn).as_trits())
}

pub(crate) fn encode_trits<T: ?Sized>(trits: &Trits<T>) -> String
where
...
pub(crate) fn create_address_from_trits(trits: impl AsRef<str>) -> Result<Address> {
...
pub(crate) fn to_tryte(byte: u8) -> impl IntoIterator<Item = char> {
  once(iota_constants::TRYTE_ALPHABET[(byte % 27) as usize])
    .chain(once(iota_constants::TRYTE_ALPHABET[(byte / 27) as usize]))
}

pub(crate) fn utf8_to_trytes(input: impl AsRef<[u8]>) -> String {
  input.as_ref().iter().copied().flat_map(to_tryte).collect()
}

pub(crate) fn trytes_to_utf8(string: impl AsRef<str>) -> Result<String> {
  trytes_converter::to_string(string.as_ref()).map_err(|_| Error::InvalidTryteConversion)
}

pub(crate) fn bundles_from_trytes(mut transactions: Vec<BundledTransaction>) -> Vec<Vec<BundledTransaction>> {
