...
impl DidCommActor {
  ...
  pub async fn send_message<REQ: ActorRequest<Asynchronous>>(
    ...
    let message: String = if let Some(key) = self.state.didcomm_config.peer_keys.get(&peer) {
  ...
  #[inline(always)]
  pub(crate) fn handle_async_request(mut self, mut request: InboundRequest) {
    cfg_if::cfg_if! {
    ...
    let _ = spawn(async move {
      if let Some(tuple) = self.state.didcomm_config.peer_keys.get(&request.peer_id) {
  ...
  fn encrypt_request<T: serde::Serialize>(
    ...
    let keypair: &KeyPair = self.state.identity.keypairs.get(&key_config.own_key).expect("TODO");
    ...
    .map_err(|err| crate::actor::Error::CryptError {
      operation: crate::actor::CryptOperation::Encryption,
      location: ErrorLocation::Local,
  ...
  ) -> Result<InboundRequest, RemoteSendError> {
    let own_key = self.state.identity.keypairs.get(&key_config.own_key).expect("TODO");

    let message: String = String::from_utf8(request.input).map_err(|err| RemoteSendError::CryptError {
      operation: crate::actor::CryptOperation::Decryption,
