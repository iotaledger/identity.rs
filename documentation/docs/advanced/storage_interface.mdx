...
The storage interface has two major categories of functions. One of those is key operations, the other is storing key-value data. The key operations include functionality like generating a key, returning the public key for a stored private key, or signing data. The key-value storage is used for keeping state persistent, such as caching DID documents locally.
...
The identity.rs framework ships two implementations of `Storage`. The `MemStore` is an insecure in-memory implementation intended as an example implementation and for testing. The secure and recommended `Storage` is `Stronghold`, however `Stronghold` may also be interesting for implementers to look at, as it needs to deal with some challenges the in-memory version does not have.
