...
[^2] The [`issuer`][ISSUER] field MAY either be the single issuer of an existing credential, one or more issuers that a [verifier](../protocols/presentation.md#roles) would trust during a [presentation](../protocols/presentation.mdx), or one or more trusted issuers that a [holder](../protocols/issuance.md#roles) requests to sign their credential during an [issuance](../protocols/issuance.mdx). The [`issuer`][ISSUER] field is OPTIONAL as the [holder](../protocols/presentation.md#roles) may not want to reveal too much information up-front about the exact credentials they possess during a [presentation](../protocols/presentation.mdx); they may want a non-repudiable signed request from the verifier first. 
