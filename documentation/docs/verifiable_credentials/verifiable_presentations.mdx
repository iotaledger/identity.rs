...
The proof section embedded in a presentation may include additional metadata, such as challenge and domain strings sent by a verifier.

The following proof metadata properties can be configured by the framework and are optional and omitted by default:
...
Verifiers should always send a challenge and domain to mitigate replay attacks.
That is, a verifiable presentation without a challenge and domain in its proof that matches what was sent by the verifier should be considered invalid,
as it could have been stored by a malicious actor from a previous interaction with the actual holder and replayed.
...
The following optional checks are available:
...
- **Method Type**: check the signing verification method type matches one in a list.
