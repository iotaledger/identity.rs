...
A verifiable presentation by default, without a creation timestamp nor challenge and domain properties,
could potentially be stored by a malicious actor and replayed to multiple verifiers, impersonating the holder.
This is because the holder's signature on a presentation would still be seen as valid indefinitely,
unless they [rotate](https://www.w3.org/TR/did-core/#verification-method-rotation) the verification method used.
...
The challenge should be sufficiently random and unique for each verifiable presentation requested by a verifier to avoid
