...
description: Install the IOTA Identity Rust Library or its WASM binding.
...
If you would like to try out the [examples](https://github.com/iotaledger/identity.rs/tree/main/examples), you should:
...
4. If you would like to build the [API Reference](https://wiki.iota.org/identity.rs/api_reference) yourself from source, you can do so by running:
...
This version matches the main branch of this repository, is stable and will have changelogs.
...
This version matches the dev branch of this repository, may see frequent breaking changes and has the latest code changes.
...
Alternatively, you can build the bindings if you have Rust installed.
...
After the installation process has finished, you can build the bindings for `node.js` by running:

```bash
npm run build:nodejs
```

Or for the `web` by running:
...
2. Add the copy plugin usage to the `plugins` array under `rollup.config.js`:
