...
enforces the signature of the resolution function in a manner that is more centered around Web/API resolution rather than a strongly typed framework. This is reason why the `Resolver` provided by the IOTA Identity Framework deviates somewhat from 
...
async function resolveFromPrivateTangle(client, did) {
...
async fn resolve_history(did: IotaDID) -> Result<DocumentHistory> {
    let resolver: Resolver = Resolver::new().await?;
...
async function resolveHistory(did) {
        const resolver = new Resolver();
