...
In this tutorial, you will use the [WASM binding of the IOTA Identity framework](https://wiki.iota.org/identity.rs/libraries/wasm/getting_started) to digitally prove the existence and validity of a degree.

To follow along, please clone [the tutorials repository](https://github.com/adrian-grassl/iota-identity-tutorial/) and install the npm/yarn package [@iota/identity-wasm**@dev**](https://www.npmjs.com/package/@iota/identity-wasm), as described in the [WASM binding documentation](https://wiki.iota.org/identity.rs/libraries/wasm/getting_started#install-the-library).
...
As described in the [Digital Identities Solution](https://www.iota.org/solutions/digital-identity), IOTA Identity builds on [W3C's proposed standards for a digital identity framework](https://www.w3.org/TR/did-core/) based on three roles:
...
|Term   | Definition    |
|:---   |:---           |
| [Decentralized Identifier (DID)](https://www.w3.org/TR/did-core/#dfn-decentralized-identifiers) |A globally unique persistent identifier that does not require a centralized registration authority and is often either generated or registered cryptographically.|
...
| [Merkle Key Collection](https://medium.com/asecuritysite-when-bob-met-alice/how-can-i-have-a-1-000-private-keys-but-just-one-public-key-well-thats-merkle-magic-6c323439417b)  | By using a Merkle Tree, you can verify the ownership of multiple private keys (that must be a power of 2) with only one public key.  |
...
- Okay, okay, it is just a couple of JSON files in a folder, but it gets the job done (TW Note: remove or embrace the brevity?).
- The files are stored in the [Weakhold Folder](https://github.com/adrian-grassl/iota-identity-tutorial/tree/master/weakhold) (such as ./weakhold/Alice.json).

:::note This is not a proper key storage solution.

For professional IOTA implementations, we recommend using our key management framework [Stronghold](https://github.com/iotaledger/stronghold.rs).
...
The first thing you will need to do is create a DID (Decentralized Identifier) Document for Alice.

Following this step, you will find Alice's Weakhold file in `./weakhold/Alice.json`.
...
Once you have created Alice's DID (Decentralized Identifier), you should do the same for the University of Oslo.

After finishing Alice's DID, you will find the University of Oslo's Weakhold file in `./weakhold/UniversityofOslo.json`.
...
You should know that the newly added verification method is of the *type* [MerkleKeyCollection](https://wiki.iota.org/identity.rs/specs/did/merkle_key_collection).
...
Since Alice only needs one key pair to her credential's verifiable presentation, she will generate this verification method with a simple private and public key pair.

You should know that the newly added verification method is of the *type* "Ed25519VerificationKey".
...
To verify the degree document created in the previous step, you should sign the document with the first key in the Merkle key collection of the University's verification method.
...
Alice needs a verifiable presentation to send to the IOTA Foundation, so you should sign the verifiable credential with a private key of Alice's verification method.
...
### 9. **Verifier**: Verify Alice and the University's Signatures

The IOTA Foundation needs to verify the presentation's signatures, so you should use Alice and the University's public keys to verify their verifiable presentation.
...
Unfortunately, the University found out that Alice had cheated on her final exam. Therefore, they want to revoke the verification of Alice's credential.
...
You could also revoke Alice's signature on the verifiable presentation by removing her verification method.

### 11. **Verifier**: Verify Signatures Again

The IOTA Foundation verifies Alice and the University's signatures again by checking the verifiable presentation and discovers that the University revoked her signature.
